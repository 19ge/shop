...
import styles from "./styles/Changetel.less";
...
export default class Changetel extends Component {
    ...
    async changepw(){
        ...
        const result=await Shop.ctel({us_tel:phone,us_pwd:pwd,sode:code});
        if(result.code ===1){
            Toast.success(result.msg,2,await function(){
    ...
    render(){
        ...
        return(
            ...
            <InputItem
                ...
                <Button type="primary" onClick={()=>this.changepw()}>确定</Button><WhiteSpace />
            </div>
        )
    }
